module SacExactTarget
  class ProspectModel < Struct.new(:prospect)
    ...
    def destroy!
      ...
      Auditory.audit(nil, self, "ExactTarget destroy took too long.", self.member, Settings.operation_types.et_timeout_destroy) if e.to_s.include?("Timeout")
      raise e
    end
    ...
    def self.find_all_by_email!(email, club_id)
      ...
      Auditory.audit(nil, self, "ExactTarget find took too long.", self.member, Settings.operation_types.et_timeout_find) if e.to_s.include?("Timeout")
      raise e
    end
