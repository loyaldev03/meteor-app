module SacExactTarget
  class ProspectModel < Struct.new(:prospect)
    ...
    def save!
      ...
      res = if subscriber.nil?
        begin 
          ...
          Auditory.audit(nil, self.prospect, e, EnrollmentInfo.find_by_prospect_id(self.prospect_id).member, Settings.operation_types.et_timeout_create) if e.to_s.include?("Timeout")
          raise e
        end
        ...
        begin
          ...
          Auditory.audit(nil, self.prospect, e, EnrollmentInfo.find_by_prospect_id(self.prospect_id).member, Settings.operation_types.et_timeout_update) if e.to_s.include?("Timeout")
          raise e
        end
      end
    ...
    def destroy!
      ...
      Auditory.audit(nil, self.prospect, e, EnrollmentInfo.find_by_prospect_id(self.prospect_id).member, Settings.operation_types.et_timeout_destroy) if e.to_s.include?("Timeout")
      raise e
    end
    ...
    def self.find_all_by_email!(email, club_id)
      ...
      Auditory.audit(nil, self.prospect, e, EnrollmentInfo.find_by_prospect_id(self.prospect_id).member, Settings.operation_types.et_timeout_find) if e.to_s.include?("Timeout")
      raise e
    end
