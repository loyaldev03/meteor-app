...
Authentication is handled using the "api_key" argument on each API call. This api_key is the authentication token of the agent that is commiting the request. If the agent is not allowed, we will stop processing the request, and proceed to return a message telling that the agent is not authorized. 
