...
class Api::MembersControllerTest < ActionController::TestCase
  ...
  def generate_post_update_terms_of_membership(member_id, terms_of_membership_id, credit_card = {}, prorated = true)
    post(:update_terms_of_membership, { :id_or_email => member_id, 
 ...
 test "Admin should enroll/create member with preferences when needs approval" do
  ...
  test "Do not allow update TOM throught API to same TOM - active member" do
    sign_in @admin_user
    @terms_of_membership_second = FactoryGirl.create :terms_of_membership_with_gateway, :club_id => @club.id, :name => "secondTom"
    @saved_member = create_active_member(@terms_of_membership, :active_member, nil, {}, { :created_by => @admin_user })
  ...
  test "Update TOM throught API - different TOM - provisional member" do
    sign_in @admin_user
    @terms_of_membership_second = FactoryGirl.create :terms_of_membership_with_gateway, :club_id => @club.id, :name => "secondTom"
    @saved_member = create_active_member(@terms_of_membership, :provisional_member, nil, {}, { :created_by => @admin_user })
    post(:update_terms_of_membership, { :id_or_email => @saved_member.id, :terms_of_membership_id => @terms_of_membership_second.id, :prorated => 0, :format => :json} )
  ...
  test "Do not allow update TOM throught API to same TOM - provisional member" do
    sign_in @admin_user
    @terms_of_membership_second = FactoryGirl.create :terms_of_membership_with_gateway, :club_id => @club.id, :name => "secondTom"
    @saved_member = create_active_member(@terms_of_membership, :provisional_member, nil, {}, { :created_by => @admin_user })
    post(:update_terms_of_membership, { :id_or_email => @saved_member.id, :terms_of_membership_id => @saved_member.terms_of_membership.id, :format => :json} )
    assert @response.body.include? "Nothing to change. Member is already enrolled on that TOM."
  end
  ...
  test "Do not allow update TOM throught API - lapsed member" do
    sign_in @admin_user
    @terms_of_membership_second = FactoryGirl.create :terms_of_membership_with_gateway, :club_id => @club.id, :name => "secondTom"
    @saved_member = create_active_member(@terms_of_membership, :applied_member, nil, {}, { :created_by => @admin_user })
    post(:update_terms_of_membership, { :id_or_email => @saved_member.id, :terms_of_membership_id => @terms_of_membership_second.id, :format => :json} )
  ...
  test "Upgrade/Downgrade Member with Basic membership level by prorate logic (NewProvisionalDays > OldProvisionalDays)- Softdecline Member (Provisional status)" do
    ...
    assert_equal @saved_member.next_retry_bill_date.utc.to_date, nbd_should_have_set.utc.to_date, "Dates: #{@saved_member.next_retry_bill_date}. Nbd: #{nbd}. nbd_should_have_set: #{nbd_should_have_set}" 
  end
