...
class FulfillmentsControllerTest < ActionController::TestCase
  ...
  test 'Agents that should get index' do
    %w[confirmed_admin_agent confirmed_fulfillment_manager_agent confirmed_agency_agent].each do |agent|
  ...
  test 'Agents should not get index' do
    %w[confirmed_supervisor_agent confirmed_representative_agent confirmed_api_agent confirmed_landing_agent].each do |agent|
      sign_agent_with_global_role(agent)
      perform_call_as(@agent) do
        get :index, partner_prefix: @partner.prefix, club_prefix: @club.name
  ...
  test 'Admin, fulfillment_managment and agency by role should not be able to create fulfillment files with fulfillments from other clubs' do
    ...
    %w[admin fulfillment_managment agency].each do |role|
