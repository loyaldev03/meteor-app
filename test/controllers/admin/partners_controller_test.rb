...
class Admin::PartnersControllerTest < ActionController::TestCase
  ...
  test 'Agents that should not get index' do
    %i[confirmed_landing_agent confirmed_supervisor_agent confirmed_representative_agent
  ...
  test 'agent with club roles should not should not get index' do
    ...
    %w[admin supervisor representative api agency fulfillment_managment landing].each do |role|
      club_role.role = role
      club_role.save
      get :index
      assert_response :unauthorized
