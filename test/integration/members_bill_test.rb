...
class MembersBillTest < ActionController::IntegrationTest
  ...
  def bill_member(enrollment_info, member)
    ...
    validate_cohort(member, enrollment_info, Transaction.last)

    within("#transactions_table .odd") do
      page.execute_script("window.jQuery('a.btn.btn-warning.btn-mini:last').click()")
    end
  ...
  test "create a member billing enroll = 0 provisional_days = 0 installment amount > 0" do
    active_merchant_stubs
    setup_member(0)
    enrollment_info = FactoryGirl.create(:complete_enrollment_info_with_cero_amount, :member_id => @saved_member.id)
    bill_member(enrollment_info, @saved_member)
  end 
