...
class EmailTemplatesTest < ActionDispatch::IntegrationTest
  ...
  def create_email_template_and_send_communication(opt_for_texts = {}, opt_for_selects = {}, _options_for_checkboxs = [])
    ...
    @tom.reload
    @saved_user = enroll_user(FactoryBot.build(:user), @tom)
  ...
  test 'do not allow to create communications days = 0 when template_type is Pillar - Logged by General Admin' do
    skip('no run now')
    setup_environment
    sign_in_as(@admin_agent)
    visit terms_of_memberships_path(@partner.prefix, @club.name)
  ...
  test 'Resend failed communication' do
    ...
    sign_in_as(@admin_agent)
    @saved_user = enroll_user(FactoryBot.build(:user), @tom)
    assert_difference('Communication.count', 1) { @saved_user.set_as_canceled! }
    communication = @saved_user.communications.last
    visit show_user_path(partner_prefix: @partner.prefix, club_prefix: @club.name, user_prefix: @saved_user.id)
    within('.nav-tabs') { click_on('Communications') }
    within('#communications') { assert page.has_no_selector? "#resend_#{communication.id}" }
