...
class MembersFulfillmentTest < ActionController::IntegrationTest
  ...
  test "every member's fulfillment with processing status sets undeliverable when member set as undeliverable" do
    setup_member(false)
    enrollment_info = FactoryGirl.build(:enrollment_info, :product_sku => 'KIT')
    ...
    fulfillment = Fulfillment.find_by_product_sku('KIT')
    fulfillment_second = FactoryGirl.create(:fulfillment, :member_id => @saved_member.id, :product_sku => 'circlet')
    fulfillment_third = FactoryGirl.create(:fulfillment, :member_id => @saved_member.id, :product_sku => 'KIT', :recurrent => true)
    fulfillment_second.set_as_processing
    fulfillment_third.set_as_processing   
    ...
    within("#report_results")do
      wait_until{
        assert page.has_content?("#{fulfillment_second.member.visible_id}")
        assert page.has_content?(fulfillment_second.member.full_name)
        assert page.has_content?((I18n.l(fulfillment_second.assigned_at, :format => :long)))
        assert page.has_content?(fulfillment_second.product_sku)
        assert page.has_content?(fulfillment_second.tracking_code)
        assert page.has_content?('processing')
    ...
    within("#report_results")do
      wait_until{
        assert page.has_content?("#{fulfillment_third.member.visible_id}")
        assert page.has_content?(fulfillment_third.member.full_name)
        assert page.has_content?((I18n.l(fulfillment_third.assigned_at, :format => :long)))
        assert page.has_content?((I18n.l(fulfillment_third.renewable_at, :format => :long)))
        assert page.has_content?(fulfillment_third.product_sku)
        assert page.has_content?(fulfillment_third.tracking_code)
        assert page.has_content?('undeliverable')
        assert page.has_content?('This address is undeliverable.')
      }
    end
    @saved_member.reload
    fulfillment_second.reload
    fulfillment_third.reload
    wait_until{
      assert_equal(@saved_member.wrong_address,'spam')
      assert_equal(fulfillment.status,'not_processed')
      assert_equal(fulfillment_second.status,'undeliverable')
      assert_equal(fulfillment_third.status,'undeliverable')
    }
  end
end
