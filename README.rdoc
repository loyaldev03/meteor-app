...
[option 1] rake maintenance:start
[option 2] touch tmp/maintenance_on
# remove phoenix from mantainance mode:
[if option 1 used] rake maintenance:end
[if option 2 used] rm tmp/maintenance_on
# deploy making sure to set maintenance mode while running migrations:
cap -S put_in_maintenance_mode="true" #{environment} deploy
