class Transaction < ActiveRecord::Base
  ...
  private
    ...
    def credit
      ...
      Auditory.report_issue("Transaction::Credit", e, {:member => self.member.inspect, :transaction => "ID: #{self.id}, response: #{self.response}"})
      response
    end
    ...
    def refund
      ...
      response = save_custom_response({ :code => Settings.error_codes.payment_gateway_error, :message => I18n.t('error_messages.airbrake_error_message') })
      Auditory.report_issue("Transaction::Refund", e, {:member => self.member.inspect, :transaction => "ID: #{self.id}, response: #{self.response}"})
      response
    end    
