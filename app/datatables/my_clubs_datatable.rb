  ...
  def data
    clubs.map do |club|
      [
        ...
        (link_to(I18n.t('activerecord.model.suscription_plans'), @url_helpers.terms_of_memberships_path(club.partner.prefix, club.name), :class => 'btn btn-mini') if @current_agent.can? :manage, TermsOfMembership, club.id).to_s
      ]
    end
  end
