  ...
  def data
    products.map do |product|
      [
        ...
        product.stock,
        product.weight,
        (link_to(I18n.t(:show), @url_helpers.product_path(@current_partner.prefix, @current_club.name, product.id), :class => 'btn btn-mini') if @current_agent.can? :read, Product, @current_club.id)+
        (link_to(I18n.t(:edit), @url_helpers.edit_product_path(@current_partner.prefix, @current_club.name, product.id), :class => 'btn btn-mini') if @current_agent.can? :edit, Product, @current_club.id)+
