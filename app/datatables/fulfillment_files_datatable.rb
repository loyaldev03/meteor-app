  ...
  def data
    files.map do |file|
      [ 
        ...
        (file.sent? ? '' : link_to("Mark as sent", @url_helpers.fulfillment_file_mark_as_sent_path(@current_partner.prefix,@current_club.name,file.id), :class => "btn btn-warning", :confirm => "Are you sure you want to mark all the fulfillments that are in progress as sent?", :id=>'mark_as_sent'))
      ]
    end
  end
