class MembersController < ApplicationController
  ...
  def search_result
    @members = Member.paginate(:page => params[:page], :per_page => 10)
                       ...
                       .with_first_name_like(params[:member][:first_name].strip)
                       .with_last_name_like(params[:member][:last_name].strip)
                       .with_address_like(params[:member][:address].strip)
                       .with_city_like(params[:member][:city].strip)
                       .with_state_like(params[:member][:state].strip)
                       .with_zip(params[:member][:zip].strip)
                       .with_email_like(params[:member][:email].strip)
  ...
  def recover
    if request.post?
      answer = @current_member.recover(@current_member.terms_of_membership, current_agent)
      if answer[:code] == Settings.error_codes.success
